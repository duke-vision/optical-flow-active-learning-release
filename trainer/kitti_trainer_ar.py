...
class TrainFramework(BaseTrainer):
    ...
    def _run_one_epoch(self):
        ...
        if self.loss_func.cfg.type == "semiflow":
        ...
        for i_step, data in enumerate(self.train_loader):
            ...
            if self.loss_func.cfg.type == "semiflow":
                ...
                masks = data['target']['mask'] if 'mask' in data['target'].keys() else None
                loss, l_sup, l_unsup, l_ph, l_sm, flow_mean = self.loss_func(flows, img_pair, gt_flows, labeled_flag, masks)
            else:
            ...
            if self.loss_func.cfg.type == "semiflow":
                ...
                key_meters.update(
                    [get_value(loss), get_value(l_ph), get_value(l_sm), 
                     get_value(flow_mean), get_value(l_atst), get_value(l_ot)],
                    labeled_flag.size(0)) 
